...
//渲染和查询
router.get("/petsKeepers",async function(req,res){
    //设置允许跨域
    ...
    let data = await client.get("/petsKeepers",{page,rows,...searchObj});
    console.log(data,00);
    res.send(data);
...
//     let data = await client.get("/petsKeepers", ...searchObj);
//     res.send(data);
    ...
    //     obj[type]=value
        ...
        // });
        // console.log(page, rows);
        // let data = await client.get("/petsKeepers", {page,rows,...obj});
        // res.send(data);
    
        // console.log(data,222222222222222)

    // } else {
    //     let data = await client.get("/petsKeepers",{page,rows});
    //     res.send(data);
...
router.post('/', async function (req, res) {
    let body = req.body;
    let data = await client.post("/petsKeepers", body);
    res.send('succ');
...
router.delete('/:id', async function (req, res) {
    ...
    console.log("shahshshshahsh",id);
    await client.delete("/petsKeepers/" + id);
    res.send('succ');
...
router.post('/change', async function (req, res) {
    ...
    let data = await client.get("/petsKeepers/" + id);
    res.send(data);
...
router.post('/confirm', async function (req, res) {
    ...
    let data = await client.put("/petsKeepers/" + id, { name: name, age: age, gender: gender,path });
    res.send(data);
...
router.get('/', async function (req, res) {
    let data = await client.get("/petsKeepers");
    res.send(data);
});
